...
-- Então use:
S.Synchronizer  = safeRequire(S.Packages:WaitForChild("Synchronizer", 5))
S.AnimalsData   = safeRequire(S.Datas:WaitForChild("Animals", 5))
S.RaritiesData  = safeRequire(S.Datas:WaitForChild("Rarities", 5))
S.AnimalsShared = safeRequire(S.Shared:WaitForChild("Animals", 5))
S.NumberUtils   = safeRequire(S.Utils:WaitForChild("NumberUtils", 5))
...
function showNotification(message, color, duration)
    ...
    if #NOTIFICATION_ACTIVE >= MAX_NOTIFICATIONS then
    -- Remova a notificação mais antiga
        local oldest = table.remove(NOTIFICATION_ACTIVE, 1)
...
local function isMyBaseAnimal(animalData)
    ...
    if sign then
        ...
        if yourBase and yourBase:IsA("BillboardGui") then
            isMine = yourBase.Enabled == true
        end
    end
    
    -- Método 2: Synchronizer (se disponível)
    if not isMine and S.Synchronizer then
        local channel = S.Synchronizer:Get(plot.Name)
        if channel then
            local owner = channel:Get("Owner")
            if owner then
                if typeof(owner) == "Instance" and owner:IsA("Player") then
                    isMine = owner.UserId == S.LocalPlayer.UserId
...
local function getAnimalHash(animalList)
    ...
    for slot, data in pairs(animalList) do
        if type(data) == "table" then
            table.insert(hashParts, string.format("%s_%s_%s", 
                tostring(slot), 
...
-- Função alternativa para estimar geração
local function estimateGeneration(animalName, mutation, traits)
    -- Valores base aproximados (ajuste conforme necessário)
    local baseValues = {
    ...
    local baseValue = baseValues[animalName] or 10000000
    ...
    if traits and type(traits) == "table" then
        baseValue = baseValue * (1 + (#traits * 0.5))
    end
...
-- ============================================================
-- NOVA INTERFACE - MODERN GUI DESIGN
-- ============================================================

local function createNovaGui()
    ...
    local function getScaleFactor()
        ...
        if isMobile and not isTablet then
            return math.clamp(scaleFactor * 0.85, 0.6, 0.9)
        elseif isTablet then
            return math.clamp(scaleFactor * 1.0, 0.8, 1.1)
        else
            return scaleFactor
        end
    end
    ...
    if CONFIG.GUI_POSITION_X and CONFIG.GUI_POSITION_Y then
        ...
        if isMobile then
            defaultPos = UDim2.new(0.5, -mainWidth/2, 0.5, -mainHeight/2)
        elseif isTablet then
            defaultPos = UDim2.new(0.5, -mainWidth/2, 0.5, -mainHeight/2)
        else
            defaultPos = UDim2.new(0, 50, 0, 50) -- TOP LEFT instead of top right
        end
    end
    ...
    local glassEffect = createElement("Frame", {
        Size = UDim2.new(1, 0, 1, 0),
        BackgroundTransparency = 0.8,
        BorderSizePixel = 0,
        Parent = mainFrame,
    ...
    local gradient = createElement("UIGradient", {
        Color = ColorSequence.new({
            ColorSequenceKeypoint.new(0, Color3.fromRGB(120, 240, 255)),
            ColorSequenceKeypoint.new(1, Color3.fromRGB(220, 120, 255))
        }),
        Rotation = 45,
    ...
    local borderGradient = createElement("UIGradient", {
        Color = ColorSequence.new({
            ColorSequenceKeypoint.new(0, Color3.fromRGB(220, 120, 255)),
            ColorSequenceKeypoint.new(0.5, Color3.fromRGB(120, 240, 255)),
            ColorSequenceKeypoint.new(1, Color3.fromRGB(220, 120, 255))
    ...
    task.spawn(function()
        while borderGradient.Parent do
            for i = 0, 360, 2 do
    ...
    createElement("UICorner", {
        CornerRadius = UDim.new(0, 16, 0, 0),
        Parent = header,
    })
    ...
    makeTextGlow(title, Color3.fromRGB(255, 140, 255), Color3.fromRGB(180, 220, 255), 1.2, 0)
    addTextGradient(title, Color3.new(0.815686, 0.305882, 1.000000), Color3.fromHSV(0.578947, 0.298039, 1.000000), 45)
    ...
    -- Tab buttons
    local tabButtons = {}
    local tabNames = {"ANIMALS", "FAVORITES", "ACTIONS", "STATS"}
    local tabIds = {"animals", "favorites", "actions", "stats"}
    
    local tabWidth = (1 / #tabNames)
    local tabTextSize = isMobile and 12 or 13
    ...
    for i, tabName in ipairs(tabNames) do
        ...
        local tabButton = createElement("TextButton", {
            ...
            Position = UDim2.new(tabWidth * (i - 1), 1, 0, 0),
            BackgroundColor3 = isActive and Color3.fromRGB(100, 60, 150) or Color3.fromRGB(40, 30, 60),
            BackgroundTransparency = isActive and 0.2 or 0.4,
            Text = tabName,
            TextColor3 = isActive and Color3.fromRGB(255, 255, 255) or Color3.fromRGB(180, 170, 200),
    ...
    if isMobile or isTablet then
        local toggleButton = createElement("TextButton", {
            ...
            BackgroundColor3 = Color3.fromRGB(100, 60, 150),
            BackgroundTransparency = 0.2,
            Text = "≡",
            TextColor3 = Color3.fromRGB(255, 255, 255),
            TextSize = 28,
        ...
        -- Add glow effect
        makeTextGlow(toggleButton, Color3.fromRGB(255, 140, 255), Color3.fromRGB(180, 220, 255), 1.2, 0)
        ...
        S.UserInputService.InputChanged:Connect(function(input)
            if toggleDragging and input.UserInputType == Enum.UserInputType.Touch then
                ...
                toggleButton.Position = UDim2.new(
                    togglePos.X.Scale,
                    togglePos.X.Offset + delta.X,
                    togglePos.Y.Scale,
                    togglePos.Y.Offset + delta.Y
                )
            end
        end)
    end
    
    -- Apply scale for mobile/tablet
    if isMobile or isTablet then
...
task.spawn(function()
    ...
    local loadSuccess = safeCall(function()
        loadConfig()
        loadFavorites()
        return true
    end, "Failed to load config/favorites")
    ...
    -- Criar GUI
    local statLabels = createNovaGui()
    
    if not statLabels then
        warn("Failed to create GUI")
        return
    end
    
    -- Inicializar scanner com delay
    task.wait(2)
    ...
    -- Setup auto-steal system
    task.wait(2)
    autoStealLoop()
    setupPromptCacheCleanup()
    
    -- Setup FPS/PING overlay
    task.spawn(function()
        ...
        local border = createElement("UIStroke", {
            Thickness = 1,
            Color = Color3.fromRGB(100, 60, 150),
            Transparency = 0.3,
            Parent = overlay,
        })
        ...
        S.RunService.RenderStepped:Connect(function()
            ...
            if now - last >= 1 then
                ...
                info.Text = string.format("FPS: %d    PING: %dms", fps, ping)
            end
        end)
    end)
    ...
    S.UserInputService.InputBegan:Connect(function(input, gameProcessed)
        ...
        if CONFIG.TOGGLE_GUI_KEYBIND and input.KeyCode == CONFIG.TOGGLE_GUI_KEYBIND then
            toggleGui()
        elseif CONFIG.TELEPORT_KEYBIND and input.KeyCode == CONFIG.TELEPORT_KEYBIND then
            teleportToHighest()
        elseif CONFIG.DESYNC_KEYBIND and input.KeyCode == CONFIG.DESYNC_KEYBIND then
            enableDesync()
        elseif CONFIG.INSTANT_CLONER_KEYBIND and input.KeyCode == CONFIG.INSTANT_CLONER_KEYBIND then
            instantCloner()
        elseif CONFIG.FLOOR_STEAL_KEYBIND and input.KeyCode == CONFIG.FLOOR_STEAL_KEYBIND then
            CONFIG.FLOOR_STEAL_ENABLED = not CONFIG.FLOOR_STEAL_ENABLED
            saveConfig()
            if CONFIG.FLOOR_STEAL_ENABLED then
                startFloorSteal()
        ...
        elseif CONFIG.KICK_SELF_KEYBIND and input.KeyCode == CONFIG.KICK_SELF_KEYBIND then
            kickSelf()
        elseif CONFIG.REJOIN_KEYBIND and input.KeyCode == CONFIG.REJOIN_KEYBIND then
            rejoinServer()
        end
    end)
    ...
    task.wait(1)
    showNotification("Nova Hub v1.0 Loaded!", COLORS.Cyan)
    showNotification("Use TAB to toggle GUI | Use config for keybinds", COLORS.Purple)
end)

print("=======================================")
print("Nova Hub v1.0 by dark_769.")
